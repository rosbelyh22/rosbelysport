...
$(document).ready(function(e) {
	...
	$("#form-add-cliente").submit(function(){
		open_loading();	
			if($("#clave").val() != $("#repetir_clave").val()){
				msj_advertencia("<strong>Cuidado!</strong> la clave y su confirmación son diferentes, revisalo bien y vuelve a intentarlo", 3000);
				close_loading();
			}else{
			$.post(base_url()+"clientes/insert", $(this).serialize(), function(data){
				if(data == "exito"){
					msj_exito("<strong>Bien!...</strong> El cliente se cargo con éxito!",3000);
					redirect("clientes/",2000);
				}else if(data == "existe"){
					msj_advertencia("<strong>Cuidado!</strong> el correo eléctronico que estas ingresando ya esta registrado en el sistema para otro cliente", 3000);
					close_loading();
				}else{
